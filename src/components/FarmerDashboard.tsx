...
export function FarmerDashboard() {
  ...
  const handleSubmitExpense = async () => {
    ...
    try {
      const totalExpenses = Object.values(newExpense.expenses)
        .map(exp => parseFloat(exp) || 0)
        .reduce((sum, exp) => sum + exp, 0);
      
      if (totalExpenses === 0) {
        setExpenseError('Please enter at least one expense amount.');
        return;
      }
      ...
      const expenseData = {
        ...
        totalExpenses,
        farmSize: parseFloat(newExpense.farmSize),
        expensePerAcre: totalExpenses / parseFloat(newExpense.farmSize),
      ...
      if (result.success) {
        setSubmittedExpenses([result.expense, ...submittedExpenses]);
        ...
        setExpenseError('Expense data submitted successfully! Admin will review it.');
      }
    } catch (err) {
      console.error('Error submitting expense:', err);
      setExpenseError(err.message || 'Failed to submit expense data. Please try again.');
    } finally {
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="expenses" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Submit Crop Expenses</CardTitle>
              <CardDescription>
                Submit your detailed farming expenses to help admin track costs and optimize agricultural programs
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {expenseError && (
                <Alert className={expenseError.includes('successfully') ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}>
                  <AlertDescription className={expenseError.includes('successfully') ? 'text-green-800' : ''}>
                    {expenseError}
                  </AlertDescription>
                </Alert>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="expenseCropCategory">Crop Category</Label>
                  <Select onValueChange={(value) => {
                    setNewExpense({...newExpense, cropCategory: value, cropType: ''});
                  }}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select crop category" />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  <Label htmlFor="expenseCropType">Specific Crop</Label>
                  <Select 
                    value={newExpense.cropType}
                    onValueChange={(value) => setNewExpense({...newExpense, cropType: value})}
                    disabled={!newExpense.cropCategory}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={newExpense.cropCategory ? "Select specific crop" : "Select category first"} />
                    </SelectTrigger>
                    <SelectContent>
                      {newExpense.cropCategory && cropCategories[newExpense.cropCategory]?.items.map((crop) => (
                        <SelectItem key={crop} value={crop}>{crop}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewExpense({...newExpense, season: value})}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    placeholder="e.g., 5"
                    value={newExpense.farmSize}
                    onChange={(e) => setNewExpense({...newExpense, farmSize: e.target.value})}
                  />
                </div>
              </div>
              
              <div className="space-y-3">
                <Label>Detailed Expenses (₹)</Label>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <div className="space-y-1">
                    <Label htmlFor="expenseSeeds" className="text-sm">Seeds</Label>
                    <Input
                      id="expenseSeeds"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.seeds}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, seeds: e.target.value}
                      })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expenseFertilizer" className="text-sm">Fertilizer</Label>
                    <Input
                      id="expenseFertilizer"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.fertilizer}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, fertilizer: e.target.value}
                      })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expensePesticides" className="text-sm">Pesticides</Label>
                    <Input
                      id="expensePesticides"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.pesticides}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, pesticides: e.target.value}
                      })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expenseLabor" className="text-sm">Labor</Label>
                    <Input
                      id="expenseLabor"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.labor}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, labor: e.target.value}
                      })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expenseIrrigation" className="text-sm">Irrigation</Label>
                    <Input
                      id="expenseIrrigation"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.irrigation}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, irrigation: e.target.value}
                      })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expenseMachinery" className="text-sm">Machinery</Label>
                    <Input
                      id="expenseMachinery"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.machinery}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, machinery: e.target.value}
                      })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expenseTransportation" className="text-sm">Transportation</Label>
                    <Input
                      id="expenseTransportation"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.transportation}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, transportation: e.target.value}
                      })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expenseStorage" className="text-sm">Storage</Label>
                    <Input
                      id="expenseStorage"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.storage}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, storage: e.target.value}
                      })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expenseOther" className="text-sm">Other</Label>
                    <Input
                      id="expenseOther"
                      type="number"
                      placeholder="0"
                      value={newExpense.expenses.other}
                      onChange={(e) => setNewExpense({
                        ...newExpense, 
                        expenses: {...newExpense.expenses, other: e.target.value}
                      })}
                    />
                  </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="notes"
                  placeholder="Any additional details about expenses, farming practices, or challenges faced..."
              ...
              <Button 
                ...
                className="w-full"
                disabled={!newExpense.cropType || !newExpense.farmSize || expenseLoading}
              >
                {expenseLoading ? (
                  <>
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" />
          ...
          {submittedExpenses.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Previously Submitted Expenses</CardTitle>
                <CardDescription>Your expense submissions that have been sent to admin</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {submittedExpenses.slice(0, 5).map((expense) => (
                    <div key={expense.id} className="border rounded-lg p-3">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-medium">{expense.cropType}</h4>
                        <Badge variant="outline">{expense.status}</Badge>
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground">
                        <span>Season: {expense.season}</span>
                        <span>Farm Size: {expense.farmSize} acres</span>
                        <span>Total Cost: ₹{expense.totalExpenses?.toLocaleString()}</span>
                        <span>Cost/Acre: ₹{expense.expensePerAcre?.toFixed(2)}</span>
                      </div>
                    </div>
                  ))}
                </div>
